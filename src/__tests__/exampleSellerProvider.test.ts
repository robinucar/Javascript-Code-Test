...
describe('ExampleSellerProvider', () => {
  ...
  it('builds the correct url for byPublisher queries', async () => {
    const http = new FakeHttpClient(jsonFixture);
    ...
    await provider.search(BookQueries.byPublisher('Penguin', 10));

    expect(http.lastUrl).toContain('/by-publisher?');
    expect(http.lastUrl).toContain('q=Penguin');
    expect(http.lastUrl).toContain('limit=10');
    expect(http.lastUrl).toContain('format=json');
  });

  it('builds the correct url for byYearPublished queries', async () => {
    const http = new FakeHttpClient(jsonFixture);
    ...
    await provider.search(BookQueries.byYearPublished(1603, 10));

    expect(http.lastUrl).toContain('/by-year?');
    expect(http.lastUrl).toContain('q=1603');
    expect(http.lastUrl).toContain('limit=10');
    expect(http.lastUrl).toContain('format=json');
  });
});
